 ...
 */
CKEDITOR.dialog.add("okimageDialog", function(editor){
	...
	function imagePreview(src){
		...
		if(src == "base64") {
			...
			if(n && "files" in n && n.files && n.files.length > 0 && n.files[0]) {
				...
				//这里上传图片到我们服务器，返回的作为imagePreviewLoad参数
				var willUploadFile = n.files[0];
				var form = new FormData();
				var xhr = new XMLHttpRequest();
				try {
					form.append('Content-Type', willUploadFile.type || 'application/octet-stream');
					// our request will have the file in the ['file'] key
					form.append('file', willUploadFile);
				} catch (err) {
					return;
				}
				...
				(new Promise(function(resolve, reject) {
					...
					var imageUploadUrlKey = editor.name+"_imageUploadUrl";
					if (window[imageUploadUrlKey]) {
						url = window[imageUploadUrlKey];
					}
					xhr.open("POST", url, true);
					xhr.send(form);
				}) ).then(function(response){

						if (response.code == -1 && response.data.imageUrls &&  response.data.imageUrls[0] ) {
							var imgSrc = response.data.cdnUrl + '/' +response.data.imageUrls[0];
							imagePreviewLoad(imgSrc);
						}
                    ...
                    },
                    {
						...
						children: [
							...
							},
							{
								type: "checkbox",
								id: "lock",
								label: editor.lang.okimage.lockRatio,
                    ...
                    },
					{
						...
						children: [
							...
							},
							{
								...
								width: "45px",
								id: "vmargin",
								label: editor.lang.okimage.vSpace
							},
							{
								type: "text",
								width: "45px",
								id: "hmargin",
								label: editor.lang.okimage.hSpace
							},
							{
								type: "text",
								width: "45px",
								id: "border",
								label: editor.lang.okimage.border
							}
						]
					}
                ]
